...
typedef tree<tiii, // key type (storing)
    null_type, // mapped type (set) -> no duplicates
    less<tiii>, // less comparator (ascending)        // the alternative is greater<int>
...
signed main() {
    ...
    int n, m; cin >> n >> m;
    os tree;
    unordered_map<int,tiii> hm;
    REP(i,1,n) {
        tree.insert({0,0,i}); // score,pen,id
        hm[i]={0,0,i};
    }
    int t,p;
