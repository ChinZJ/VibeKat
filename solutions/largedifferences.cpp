...
signed main() {
     ...
     * TODO
     * 
     * To maximize the minimum difference between adjacent elements,
     *      fundamentally want to avoid placing two "small" or "large" numbers next to each other
     * 
     * Imagine any final valid arrangmenet with alternating colors
     *      Every adjacent pair consists of one black and one white element
     *      To maximize difference, one color should hold the smaller half of the sorted array, and the other the larger half
     * 
     * Sort the array, split into small and big, by letting d= (floor)N/2
     *      The maximum "index gap" we can consistently maintain between paired elements from the Small and Big is d
     *      
     * 
     * The construction will be A[d], A[0], A[d+1], A[1], ...
     * 
     * Case 1: N is even - then only one optimal way to split Smalls and Bigs
     * Case 2: N is odd - one extra element
     *      Find the second smallest difference
     * 
     *      We can place k+1 elements to leave gaps between them
     *      We can form k+1 different such contiguous elements of size k + 1
     */
    ...
    if (N%2 == 0) {
        ...
        int ans = INT_MAX;
        FOR(i,0,d) {
            ans = min(ans, arr[i+d]- arr[i]);
        }
        cout << ans << '\n';
    } else {
        vi diffs; diffs.reserve(d+1);
        REP(i,0,d) {
            diffs.pb(arr[i+d] - arr[i]);
        }
        sort(ALL(diffs));
        cout << diffs[1] << '\n';
    }
