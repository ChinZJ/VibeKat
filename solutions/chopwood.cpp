...
signed main() {
    ...
    FOR(i,0,n) {
        cin >> v;
        seq[i]=v;
        if (hm.find(v)==hm.end()) hm[v]=1;
        else hm[v]++;
    }

    priority_queue<int, vi, greater<int>> q;
    REP(i,1,n+1) {
        if (hm.find(i)==hm.end()) q.push(i);
    }
    ...
    FOR(i,0,n) {
        ...
        if (hm.count(lastVtx) && hm[lastVtx]==1 
                && lastVtx<q.top() && lastVtx!=seq[i]) { cout << "Error\n"; return 0;}

        ss << q.top() << '\n'; q.pop();
        hm[seq[i]]--;
        if (hm[seq[i]]==0) {
            q.push(seq[i]); hm.erase(seq[i]);
        }
    }
    cout << ss.str();
