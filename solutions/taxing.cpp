...
signed main() {
    ...
    int B; cin >> B;
    vector<ld> lowb(B+2,0), rate(B+1,0);
    ld s,p;
    REP(i,1,B) {
        cin >> s >> p;
        lowb[i]=lowb[i-1]+s;
    ...
    ld P; cin >> P;
    lowb[B+1]=1e15; rate[B]=P/100.0;

    int F; cin >> F;
    ld e,m;
    while (F--) {
        cin >> e >> m;
        int k=upper_bound(lowb.begin(), lowb.end(), e) - lowb.begin() - 1;

        ld gift=0, rem=m;
        for (int i=k; i<=B && rem>0; i++) {
            ld cap=lowb[i+1]-max(e,lowb[i]);
            ld afttax=cap*(1-rate[i]);
        ...
        cout << fixed << setprecision(8) << gift << '\n';
    }
}
