...
signed main() {
    ...
    FOR(i,0, (1<<n)) {
        int mask=i;
        bool ok=true;
        for (int pos = LSOne(mask); pos != 0; pos = (LSOne(mask))) {
            mask ^= pos;
        ...
        if (ok) isindep[i]=true;
    }

    vi memo((1<<n), INT_MAX); memo[0] = 0;
    vi par((1<<n), INT_MAX); par[0] = 0;
    FOR(i,0,(1<<n)) {
        // if (__builtin_popcount(i) > c) continue;

        for (int sub = i; sub > 0; sub = (sub - 1) & i) {
            if (__builtin_popcount(sub) > c) continue;
            if (isindep[sub]) {
                int val = memo[i ^ sub] + 1;
                if (val < memo[i]) {
                    memo[i] = val;
                    par[i] = sub;
                }
