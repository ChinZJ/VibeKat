...
void solve(int st, int end) {
    if (st > end) return;
    int r = end - 1;
    FOR(i, st, end) {
        int val = t[i];
        if (vis[val]) {
            if (i <= pla[val].se && i >= pla[val].fi) continue;
            else {
                cout << "IMPOSSIBLE\n";
                impo = true;
                return;
        ...
        vis[val] = 1;
        ans++;
        pla[val] = {i, min(maxpos[val], r)};
        ss << i << ' ' << min(maxpos[val], r) << ' ' << val << '\n';
