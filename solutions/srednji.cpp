...
signed main() {
    ...
    int N,B; cin >> N >> B;
    vi left,right,parright;
    left.reserve(N);right.reserve(N);parright.reserve(N);
    bool got=false;
    int tmp;
    int ridx=0;
    FOR(i,0,N) {
        ...
        if (tmp==B) {
            left.pb(tmp);
            parright.pb(0); right.pb(tmp);
            got=true; continue;
        }
        if (got) {
            right.pb(tmp);
            (tmp<B) ? parright.pb(-1 + parright[ridx++]) : parright.pb(1+ parright[ridx++]);
        }
        else {
            left.pb(tmp);
        }
    ...
    vi parleft(sz(left),0);
    REF(i,sz(left)-1,0) {
        if (left[i]==B) {
            continue;
        } elif (left[i] > B) {
            parleft[i]=1+parleft[i+1];
        } else {
            parleft[i]=-1+parleft[i+1];
        }
    }

    unordered_map<int,int> rightsideodd, rightsideeven;
    FOR(i,0,sz(parright)) {
        if (i%2) {
            if (rightsideodd.find(parright[i])==rightsideodd.end()) rightsideodd[parright[i]]=1;
            else rightsideodd[parright[i]]++;
        } else {
            if (rightsideeven.find(parright[i])==rightsideeven.end()) rightsideeven[parright[i]]=1;
            else rightsideeven[parright[i]]++;
        }    
    }
    ...
    REF(i,sz(left)-1,0) {
        if ((sz(left)-i-1)%2) {
            ans+=rightsideodd[-parleft[i]];
        } else {
            ans+=rightsideeven[-parleft[i]];
        }
    }
    cout << ans << '\n';
}