...
signed main() {
     ...
     * TODO
     * 
     * Binary Indexed Tree Fused with Binary Search
     * 
     * Each element A_i has a cost B_i.
     * Goal is to delete some elements so that the remaining array has one value that is 
     *      1. simultaneously the largest and 
     *      2. appears more than half the time
     * 
     * General algorithm
     * For each distinct value v (smallest to largest)
     *      1. Pool already contains all elements A_i < v
     *      2. Keep all k copies of v
     *      3. Keep best (k-1) from pool
     *      4. Delete everything else
     *      5. Add v elements to pool for future iterations
     * 
     * Eg.  A = [3, 1,  3, 2, 3]
     *      B = [5, 10, 2, 8, 1]    totcost = 26
     *      Try v=1 (k=1, costs=[10])
     *          Elements A_i > (v = 1) must be deleted (all 3's and 2's) 
     *          Elements with A_i < (v = 1) none exist, keep 0 others
     *          Best kept = 10 + 0 = 10, cost = 26 - 10 = 16
     * 
     *      Try v=2 (k=1, costs=[8])
     *          Delete everything A_i > v=2
     *          Pool of elements with A_i < 2 = {10} can keep at most k-1 = 0 of them
     *          Best kept = 8 + 0 = 8, cost = 26 - 8 = 18
     * 
     *      Try v=3 (k=3, costs=[1,2,5])
     *          Elements with a_i > (v=3) none exist
     *          Pool of elements with A_i < (v=3) = {10, 8} can keep at most k-1 = 2 of them
     *          Best kept = 8 + 18 = 26, cost = 26 - 26 = 0
     * 
     * Necessity for a BIT arises from the fact that the pool cannot be sorted everytime we query top k-1 
     *      
     */
    ...
    vi a(n), b(n);
    FOR(i,0,n) cin >> a[i];
    FOR(i,0,n) {cin >> b[i]; totSum += b[i];}

    int MAXB = 300'001;

    // Two BTIS indexed by cost value
    // cntBit: how many elements with this cost are in the pool
    // sumBit: sum of costs at this position
    PURQ  cntBit(MAXB), sumBit(MAXB);
    int poolCnt = 0; ll poolSum = 0;

    map<int, vi> groups;
    FOR(i,0,n) groups[a[i]].pb(b[i]);

    ll ans = LLONG_MAX;

    for (auto& [v,costs] : groups) {
      int k = sz(costs);
      ll groupSum = 0;
      for (int c : costs) groupSum += c;
      ...
      if (want <= 0) {
        ...
        // Everything above boundary is definitely in the top k
        // Everything below is definitely not
        // Tricky part is elements = boundary (some may be in top k, some may not)
        int boundary = cntBit.select(poolCnt - want);

        ll cntBelow = cntBit.rsq(boundary - 1);
        ll sumBelow = sumBit.rsq(boundary - 1);

        ll needFromBoundary = (poolCnt - want) - cntBelow;

        ll bottomSum = sumBelow + (needFromBoundary * boundary);

        topKSum = poolSum - bottomSum;
      }

      ans = min(ans, totSum - (groupSum + topKSum));

      for (int c : costs) {
        cntBit.update(c,1);
        sumBit.update(c, (ll)c);
        poolCnt++; poolSum += c;
      }
    }
    cout << ans << '\n';
