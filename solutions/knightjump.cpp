...
signed main() {
    ...
    for (int i=0; i<N; ++i) {
        ...
        for (int j=0; j<N; ++j) {
            if (s[j]=='.') continue;
            elif (s[j]=='K') {kx=i, ky=j; grid[i][j]=0;}
            else grid[i][j]=-1;
        }
    }

    queue<pii> bfs; bfs.push({kx,ky});
    ll ans=0; bool ok=false;

    array<pii, 8> mv{{{2,1}, {2,-1}, {-2,1}, {-2,-1}, {1,2}, {1,-2}, {-1,2}, {-1,-2}}};
    while (!bfs.empty()) {
        auto [xx,yy] = bfs.front(); bfs.pop();
        ...
        for (auto [nx,ny] : mv) {
            int nnx=nx+xx, nny=ny+yy;
            if (nnx>=0 && nnx<N && nny>=0 && nny<N && grid[nnx][nny]==0) {
                grid[nnx][nny]=grid[xx][yy]+1;
                bfs.push({nnx,nny});
            }
        }
    }
