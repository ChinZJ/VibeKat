...
signed main() {
    ...
    while (cin >> n && n!=0) {
        ...
        int t, intd; double d;
        FOR(i,0,n) {
            cin >> t >> d; intd=(int)round(d*100);
            arr[t]+=intd;
        }
        
        int maxsum=INT_MIN, maxst=0, maxend=0;
        int currsum=0,currst=0;

        REP(i,0,t) {
            currsum+=arr[i];
            if (currsum==maxsum && (maxend-maxst)>(i-currst)) {
                maxst=currst; maxend=i;
            }
            if (currsum>maxsum) {
                maxsum=currsum; maxst=currst; maxend=i;
            }
        ...
        if (maxsum<=0) {
            ...
            double profit=maxsum/100.0;
            cout << fixed << setprecision(2) << profit << ' ' << maxst << ' ' << maxend << '\n';
        }
    }
