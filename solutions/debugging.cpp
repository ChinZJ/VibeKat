...
signed main() {
    ...
    unordered_map<ll, ll> memo; memo.reserve(100001);
    memo[1] = 0;

    auto solve = [&](auto& self, ll nn) -> ll {
        if (auto it = memo.find(nn); it != memo.end()) return it-> second;

        ll best = (nn-1)*p + r; // base case v == 1

        ll k = 2;
        while (k <= nn) {
            ll v = (nn+k-1) / k; // worst partition has v lines
            ll kmin = (nn+v-1) / v; // multiple k values lead to same v by ceiling function
            ll cost = (kmin-1) * p + r + self(self, v);
            best = min(best, cost);

            if (v==1) break;
            k = nn / (v-1) + 1; // take the next largest k which will give a unique value
                                // overshooting is accounted for by recalculating kmin
                                // Note that v==1 cannot be reached, thus the initialization above
        }
        return memo[nn] = best;
    };
