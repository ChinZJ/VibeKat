...
signed main() {
    ...
    unordered_map<int, pii> best; // score, {cost,id}
    best[1]={1,1};
    ...
    FOR(i,0,N) {
        int s,d;
        cin >> s >> d;
        if (d>T) continue; // exceed upperbound
        if (s>0 && d>=s) continue; // better off with {1,1}
        auto it=best.find(s);
        if (it==best.end() || d < it->se.fi) {
            best[s] = {d, i+2};
        }
        if (s>0) maxs=max(maxs,s);
    }

    struct Cheer {int s,d,id; };
    vector<Cheer> cheers; cheers.reserve(sz(best));

    for (auto& [s, pd]: best) {
        cheers.pb({s,pd.fi, pd.se});
    }

    int C=sz(cheers);
    int hi=T+maxs;
    vi dist(hi+1,T+1); dist[0]=0;
    vi par(hi+1,-1);
    vi parid(hi+1,-1);
    ...
    REP(d,0,T) {
        FOR(bi,0,sz(buck[d])) {
            ...
            FOR(ci,0,C) {
                ...
                dist[v]=nd;
                par[v]=u;
                parid[v]=cheers[ci].id;
                buck[nd].pb(v);
            }
        }
        // save space
        buck[d].clear();
        buck[d].shrink_to_fit();
    }

    vi seq; seq.reserve(200000);
    for(int cur=T; cur!=0; cur=par[cur]) seq.pb(parid[cur]);

    cout << sz(seq) << '\n';
    for (int aa : seq) cout << aa << ' ';
    cout << '\n';
}