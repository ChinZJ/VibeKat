...
signed main() {
    ...
    vector<string> target; target.reserve(n);
    FOR(i,0,n) {
        cin >> s1;
        if (hm.find(s1[0])==hm.end()) hm[s1[0]]=1;
        else hm[s1[0]]++;

        if (s1[0]==end) target.pb(move(s1));
    }

    string ans = "123";
    for (auto& test : target) {
        int eid = sz(test)-1;
        if (hm[test[eid]] == 0 ||
            (hm[test[eid]] == 1 && test[0]==test[eid]) ) {
                cout << test <<  "!\n"; return 0;
        } elif (ans=="123") ans=test;
    }
    if (ans=="123") cout << "?\n";
    else cout << ans << '\n';
