...
signed main() {
     ...
     * TODO
     * 
     * BSTA + Independent set
     * Independent set because we are using Dinic to find "bad pairings"
     *      For each bad pairing we can put at most one egg
     *      Thus formula is B + R - max flow 
     */


    int N,B,R; cin >> N >> B >> R;

    vector<pair<ll,ll>> cB(B);
    FOR(i,0,B) {
        ll x,y; cin >> x >> y;
        cB[i] = {x,y};
    }
    vector<vector<pair<int, long double>>> adj(R);
    FOR(i,0,R) {
        ll x,y; cin >> x >> y;

        FOR(j,0,B) {
            auto [x1,y1] = cB[j];
            long double d1 = (long double)(x) - (long double)x1;
            long double d2 = (long double)(y) - (long double)y1;
            long double diff = sqrt((d1*d1) + (d2*d2));
            adj[i].pb({j, diff});
        }
    }

    int src = B+R+1, snk=src+1;
    ...
    while ((hi-lo) > EPS) {
        ...
        FOR(i,0,R) {
            ...
            for (auto& [j, diff] : adj[i]) {
                if ((diff - mid) <= EPS ) flow.addEdge(i, R+j, 1);
            }
        }
        FOR(i,R,R+B+1) flow.addEdge(i, snk, 1);

        if (B + R - flow.calc(src,snk) >= N) {
            lo = mid;
        } else hi=mid;
    }
