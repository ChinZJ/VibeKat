...
struct DSU {
    vi e;  // e[i] < 0: i is root with size -e[i]; e[i] >= 0: parent of i
    
    DSU(int n) : e(n, -1) {}
    
    int find(int x) { 
        return e[x] < 0 ? x : e[x] = find(e[x]);  // path compression
    }
    ...
    bool unite(int a, int b) {  // returns true if unite happened
        a = find(a), b = find(b); // since both are roots their value should be negative
        if (a == b) return false;
        if (e[a] > e[b]) swap(a, b);  // unite by size: a is larger
        e[a] += e[b]; e[b] = a; // root stays negative, e[b] now >= 0 and points to its parent
        return true;
    }
};
