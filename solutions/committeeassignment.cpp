...
signed main() {
    ...
    /**
     * NP Hard
     * Chromatic numbering: minimum number of colors required to color 
     * the vertices of a graph such that no two adjacent vertices share the same color
     * 
     * Let n be nodes, and form edge uv if they have a bad relationship.
     * Then we are interested in minimizing the number of independent sets
     */
    ...
    while (cin >> n >> m) {
        ...
        if (m==0) {cout << 1 << '\n'; continue;}
        int maxMask=(1<<n);
        memset(dp,0,sizeof(int)*maxMask); memset(independent,0,sizeof(bool)*maxMask);

        unordered_map<string,int> name;
        int uid=0;
        int bad[16]; memset(bad,0,sizeof(bad));
        FOR(i,0,m) {
            cin >> s1 >> s2;
            ...
            id1=name[s1]; id2=name[s2];

            setBit(bad[id1],id2); setBit(bad[id2],id1);
        }
        ...
        REP(mask,1,(1<<n)-1) {
            ...
            int rest=mask^(1<<v);
            independent[mask]=independent[rest] && !(bad[v] & rest);
        }
