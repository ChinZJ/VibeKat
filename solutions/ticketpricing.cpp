...
signed main() {
    ...
    int N,W; cin >> N >> W;
    int K,k,tmpk, qty; ll p,val, pot;

    set<int> ans;

    vector<vll> dp(N+1, vll(W+2,-1)); dp[0][0]=0;
    vector<vll> used(N+1, vll(W+2,-1));
    bitset<301> mask; mask[0]=1;

    int parr[1000],karr[1000];
    ...
    REP(i,1,W+1) {
        ...
        FOR(ii,0,K) {
            p=parr[ii], k=karr[ii];
            for (int chk = mask._Find_first(); chk < mask.size(); chk = mask._Find_next(chk)) {
                qty=(int)chk+k;
                tmpk=k;
                if (qty>N) {
                    qty=N; tmpk=N-chk;
                ...
                if (dp[chk][i-1]!=-1) {
                    val=dp[chk][i-1]+(p*tmpk);
                    pot=(i==1) ? p : used[chk][i-1];
                    // cout << "val " << val << '\n';
                    if (val>dp[qty][i]) {
                        // cout << "putting in " << qty << '\n';
                        mask[qty]=1;
    ...
    ll maxrev=-1,bstprice=LLINF;
    REP(q,0,N) {
        if (dp[q][W+1]==-1) continue;
        if (dp[q][W+1]>maxrev) {
            maxrev=dp[q][W+1];
            bstprice=used[q][W+1];
        } elif (dp[q][W+1]==maxrev) bstprice=min(bstprice,used[q][W+1]);
    }
    cout << maxrev << '\n' << bstprice << '\n';
}