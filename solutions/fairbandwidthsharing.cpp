...
signed main() {
     ...
     * TODO
     * 
     * Requirement for BSTA is that the function that you are searching on must be monotonic. 
     * 
     * The unfairness measure $\sum \frac{(x_i - y_i)^2}{y_i}$. 
     *      The best way to minimize this is to trivially set $x_i = y_i$. 
     *      The only issue being that the question clamps extreme values such that $a_i \leq x_i \leq b_i$
     * 
     * Thus consider a small test case where we have extra units from the clamping of values. 
     *      Then it is always ideal to spread out the extra units proportionately to all other species where possible. 
     *      (The numerator of the unfairness measure is a sum of squared deviations)
     * 
     * Thus, by simply running BSTA on a scalar c * y_i = x_i, we can perform the "optimal spreading"
     */

    int n; double t; cin >> n >> t;

    double a,b,d, totd=0;
    vector<double> as(n), bs(n), ds(n);
    FOR(i,0,n) {
        cin >> as[i] >> bs[i] >> ds[i];
        totd += ds[i];
    }
    ...
    vector<double> y(n);
    FOR(i,0,n) y[i] = t * ds[i] / totd;

    double lo=0, hi=1e18;
    FOR(ii,0,100) { // Given (hi - lo)/2^k, where k = 100 the range is 10e-12
