...
ll mergehlp(vi& arr, int l, int mid, int r) {
    vi larr(arr.begin() + l, arr.begin() + mid + 1);
    vi rarr(arr.begin() + mid + 1, arr.begin() + r + 1);
    ...
    while (i < larr.size()) arr[k++] = larr[i++];
    while (j < rarr.size()) arr[k++] = rarr[j++];

    return inv;
}


ll mergest(vi& arr, int l, int r) {
    ll inv=0;
    if (l<r) {
        int mid = (l+r)/2;
        inv += mergest(arr,l,mid); // sort first half
        inv += mergest(arr,mid+1,r); // sort second half
        inv += mergehlp(arr, l, mid, r); // merge both
    }
    return inv;
}
