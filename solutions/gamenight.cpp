...
signed main() {
    ...
    vi prefA(2*n+1,0), prefB(2*n+1,0), prefC(2*n+1,0);
    FOR(i,0,2*n) {
        if(i<n) {
            if (t[i]=='A') cA++;
            elif (t[i]=='B') cB++;
        ...
        prefA[i+1]=prefA[i]+(t[i]=='A');
        prefB[i+1]=prefB[i]+(t[i]=='B');
        prefC[i+1]=prefC[i]+(t[i]=='C');
    }
    ...
    // ABC
    FOR(st,0,n) {
        int match = (prefA[st+cA] - prefA[st])
                + (prefB[st+cA+cB] - prefB[st+cA])
                + (prefC[st+cA+cB+cC] - prefC[st+cA+cB]);
    ...
    FOR(st,0,n) {
        int match = (prefA[st+cA] - prefA[st])
                + (prefC[st+cA+cC] - prefC[st+cA])
                + (prefB[st+cA+cB+cC] - prefB[st+cA+cC]);
