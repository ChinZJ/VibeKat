...
signed main() {
    ...
    int perSide=N/2;
    double spaces=(double)L / (perSide-1);
    vector<double> T(perSide);
    FOR(i,0,perSide) {
        T[i]=i*spaces;
    }

    // Total trees, number on left
    vector<vector<double>> dp(N+1, vector<double>(perSide+1, 1e18));
    dp[0][0]=0.0;

    FOR(i,0,N) {
        REP(j,0,min(i,perSide)) {
            ...
            // Send to left side
            if (j < perSide) {
                double cost = abs(pos[i] - T[j]);
                dp[i+1][j+1]=min(dp[i+1][j+1], dp[i][j]+cost);
            }

            // Send to right
            if (onRight < perSide) {
                double cost=sqrt((pos[i] - T[onRight]) * (pos[i] - T[onRight]) + (double)W * W);
                dp[i+1][j] = min(dp[i+1][j], dp[i][j] + cost);
            }
        }
    }
    cout << fixed << setprecision(10) << dp[N][perSide] << '\n';
}