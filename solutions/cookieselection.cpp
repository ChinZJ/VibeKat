...
typedef tree<pii, // key type (storing)
    ...
    less<pii>, // less comparator (ascending)        // the alternative is greater<int>
    rb_tree_tag, // red black tree implementation
    tree_order_statistics_node_update> ost; // order statistics operations
...
signed main() {
    ...
    while (cin >> s) {
        if (s=="#") {
            sze=sz(tree);
            auto [a,b]=*tree.find_by_order(sze/2);
            cout << a << '\n';
            tree.erase({a,b});
        } else {
            val=stoi(s);
