...
unordered_set<string> mapper{"h", "he", "li", "be", "b", "c", "n", "o", "f", "ne",
...
signed main() {
    ...
    while (T--) {
        ...
        while (!pos.empty()) {
            ...
            if (cur+1 <= n && mapper.count(s.substr(cur,1)) && !ok[cur+1]) {
                // cout << "move " << cur << ' ' << cur+1 << '\n';
                ok[cur+1]=true; pos.push(cur+1);
            }
            if (cur+2 <= n && mapper.count(s.substr(cur,2)) && !ok[cur+2]) {
                // cout << "move " << cur << ' ' << cur+2 << '\n';
                ok[cur+2]=true; pos.push(cur+2);
            }
        }
        cout << (ok[n] ? "YES\n" : "NO\n");
    }
