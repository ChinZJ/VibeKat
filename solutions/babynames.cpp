...
typedef tree<string, 
    null_type, // value pair
    less<string>, // comparator (ascending)        // the alternative is greater<int>
    rb_tree_tag, 
    tree_order_statistics_node_update> oms; // sorting is still done wrt key, not its pair

oms males,females;
unordered_map<string,int> gen;

void addName(const string& name, int g) {
    auto& ms=(g==1) ? males : females;
    ms.insert(name);
}

void delName(const string& name) {
    int g=gen[name];
    auto& ms=(g==1) ? males : females;
    ms.erase(name);
}

ll query(const string& s1, const string& s2, int g) {
    ll tot=0;
    int ls1=sz(s1);
    string ss2;
    if (sz(s2) > ls1) {
        char nxt=s2[ls1-1]+1;
        ss2=s2.substr(0,ls1-1) + nxt;
    } else {
        ss2=s2;
    }
        
    if (g==1 || g==0) {
        tot+=males.order_of_key({ss2,0}) - males.order_of_key({s1,0});
    } if (g==2 || g==0) {
        tot+=females.order_of_key({ss2,0}) - females.order_of_key({s1,0});
    }
    return tot;
}
