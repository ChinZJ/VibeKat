...
signed main() {
     ...
     * TODO
     * 
     * BSTA on the initial tolerance
     *      Perform linear scan on the line segment for a valid starting slice
     *      If we start at a slice and expand left and right but get stuck, 
     *      all other starting slices in that trapped interval NEED NOT BE TESTED (they will reach the same boundaries)
     */
    ...
    auto check=[&](ll val)->bool {
        ...
        while (expand) {
            expand=false;
            while (lp != rp && val >= p[lp]) {
                expand=true;
                val += p[lp];
                lp = (lp-1+n)%n;
            }
            while (lp != rp && val >= p[rp]) {
                expand=true;
                val += p[rp];
                rp = (rp+1+n)%n;
            }
