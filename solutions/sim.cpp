...
signed main() {
    ...
    while(t--) {
        // ] goes to end
        // [ goes to start
        // < backspace but does nothing if at start
        ...
        while(i<n) {
            ...
            if (c=='[') {
                ...
                while(i<n && s[i]!='[' && s[i]!= ']') {
                    if (s[i]=='<') {
                        if ((int)str2.size()) str2.pop_back();
                    } 
                    else str2.pb(s[i]);
                    i++;
                }
                if((int)str2.size()) dq.push_front(move(str2));
            } elif (c=='<') {
                while((int)dq.size() > 1 && (int)dq.back().size() == 0) dq.pop_back();
                if (dq.back().size() != 0) dq.back().pop_back(); 
                i++;
            } elif (c==']') {
                i++;
            } else {
