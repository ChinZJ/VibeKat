#include<bits/stdc++.h>
using namespace std;

typedef long long ll;
typedef long double ld;
typedef pair<int, int> pii;
typedef vector<int> vi;
typedef vector<ll> vll;
typedef tuple<int, int, int> tiii;

#define pb push_back
#define fi first
#define se second

#define elif else if

#define sz(x) (int)(x).size()

#define FOR(i, a, b) for (int i = int(a); i < int(b); i++)
#define REP(i, a, b) for (int i = int(a); i <= int(b); i++)
#define REF(i, a, b) for (int i = int(a); i >= int(b); i--)

#define ALL(x) (x).begin(), (x).end()
#define RALL(x) (x).rbegin(), (x).rend()

// Multiple by 2 use <<
// Divide by 2 use >>
#define isOn(S, j) (S & (1<<j))
#define setBit(S, j) (S |= (1<<j))                   // Turns ON bit j
#define clearBit(S, j) (S &= ~(1<<j))                // Turns OFF bit j
#define toggleBit(S, j) (S ^= (1<<j))                // Toggles bit j
#define LSOne(S) (S & (-S))                          // Finds least significant bit, returns 2^j
#define setAll(S, n) (S = (1<<n)-1)                  // turn ON ALL bits of size n

#define modulo(S, N) ((S) & (N-1))                   // returns S % N, where N is a power of 2 (2^N)
#define isPowerOfTwo(S) (!(S & (S-1)))               // Checks if S is a power of 2
#define nearestPowerOfTwo(S) (1<<lround(log2(S)))    // Finds the nearest power of 2
#define turnOffLastBit(S) ((S) & (S-1))              // Turns OFF the last bit
#define turnOnLastZero(S) ((S) | (S+1))              // Turns ON the last zero
#define turnOffLastConsecutiveBits(S) ((S) & (S+1))  // Turns OFF the last consecutive bits
#define turnOnLastConsecutiveZeroes(S) ((S) | (S-1)) // Turns ON the last consecutize zeroes

// __builtin_popcount(S): counts number of bits that are turned on in S
// __builtin_popcountll(S): ll version
// __builtin_ctz(S): counts number of trailing zeroes in S
// __builtin_clz(S): counts number of leading zeroes in S 

// 32bit ~ 2x10^9
// 64bit ~ 9x10^18

const int INF = INT_MAX;
const ll LLINF = LLONG_MAX;

// overloads cout for array printing
template<typename T> ostream& operator<<(ostream& out, vector<T>& a) {
    for(auto &x : a) out << x << ' ';
    return out;
}; // allows cout << arr << "\n";

// From https://codeforces.com/blog/entry/62393
struct custom_hash {
    static uint64_t splitmix64(uint64_t x) {
        // http://xorshift.di.unimi.it/splitmix64.c
        x += 0x9e3779b97f4a7c15;
        x = (x ^ (x >> 30)) * 0xbf58476d1ce4e5b9;
        x = (x ^ (x >> 27)) * 0x94d049bb133111eb;
        return x ^ (x >> 31);
    }

    size_t operator()(uint64_t x) const {
        static const uint64_t FIXED_RANDOM = chrono::steady_clock::now().time_since_epoch().count();
        return splitmix64(x + FIXED_RANDOM);
    }
};

// Example use: unordered_map<long long, int, custom_hash> safe_map;

ll gcd(ll a, ll b) {
    while (b) {
        a %= b;
        swap(a, b);
    }
    return a;
}

ll lcm(ll a, ll b) {
    return (a / gcd(a, b) * b);
}

ll pow(ll a, ll b) {
    ll res = 1; 
    while (b > 0) {
        if (b & 1) {
            res = res * a;
        }
        a = a * a;
        b >>= 1;
    }
    return res;
}

// Read K values and ignore the rest
// cin >> a >> b >> c;
// cin.ignore(numeric_limits<streamsize>::max(), '\n');
// Use cin.ignore() before getline()

// cout << fixed << setprecision(8) << ... for decimal precision


// cout << setw(10) << 42 << "\n";           // "        42" (right-aligned)
// cout << left << setw(10) << 42 << "\n";   // "42        " (left-aligned)
// cout << setfill('0') << setw(5) << 42 << "\n";  // "00042" (zero-padded)

ll MOD=1000000007;

signed main() {
    cin.tie(0) -> sync_with_stdio(0);

    int V,E,Q; cin >> V >> E >> Q;
    vector<bitset<4005>> adjMtx(4005);
    vector<bitset<4005>> transAdjMtx(4005);
    vector<bitset<4005>> compAdjMtx(4005, bitset<4005>().set());
    vector<bitset<4005>> compTransAdjMtx(4005, bitset<4005>().set());
    FOR(i,0,4005) {
        compAdjMtx[i].reset(i);
        compTransAdjMtx[i].reset(i);
    }
    int u,v;
    bool flip=false; bool comp=false;
    FOR(i,0,E) {
        cin >> u >> v;
        adjMtx[u].flip(v);
        compAdjMtx[u].flip(v);

        transAdjMtx[v].flip(u);
        compTransAdjMtx[v].flip(u);
    }

    int cmd;
    FOR(i,0,Q) {
        cin >> cmd;
        if (cmd==1) { // add vertex
            if (comp) {
                adjMtx[V].set();
                transAdjMtx[V].set();
                compAdjMtx[V].reset();
                compTransAdjMtx[V].reset();
                FOR(j,0,V) {
                    adjMtx[j].set(V);
                    transAdjMtx[j].set(V);

                    compAdjMtx[j].reset(V);
                    compTransAdjMtx[j].reset(V);
                }
            } else {
                compAdjMtx[V].set();
                compTransAdjMtx[V].set();
                adjMtx[V].reset();
                transAdjMtx[V].reset();
                FOR(j,0,V) {
                    compAdjMtx[j].set(V);
                    compTransAdjMtx[j].set(V);

                    adjMtx[j].reset(V);
                    transAdjMtx[j].reset(V);
                }
            }
            V++;
        } elif (cmd==2) { // new dir edge
            cin >> u >> v;
            if (flip) swap(u,v);

            if (comp) {
                adjMtx[u].reset(v);
                compAdjMtx[u].set(v);

                transAdjMtx[v].reset(u);
                compTransAdjMtx[v].set(u);
            } else {
                adjMtx[u].set(v);
                compAdjMtx[u].reset(v);

                transAdjMtx[v].set(u);
                compTransAdjMtx[v].reset(u);
            }
        } elif (cmd==3) { // delete all incoming and outgoing
            cin >> u;
            if (comp) {
                FOR(i,0,4005) {
                    if (i==u) {
                        adjMtx[i].set();
                        compAdjMtx[i].reset();

                        transAdjMtx[i].set();
                        compTransAdjMtx[i].reset();
                    } else {
                        adjMtx[i].set(u);
                        compAdjMtx[i].reset(u);

                        transAdjMtx[i].set(u);
                        compTransAdjMtx[i].reset(u);
                    }
                }
            } else {
                FOR(i,0,4005) {
                    if (i==u) {
                        adjMtx[i].reset();
                        compAdjMtx[i].set();

                        transAdjMtx[i].reset();
                        compTransAdjMtx[i].set();
                    } else {
                        adjMtx[i].reset(u);
                        compAdjMtx[i].set(u);

                        transAdjMtx[i].reset(u);
                        compTransAdjMtx[i].set(u);
                    }
                }
            }   
        } elif (cmd==4) { //remove X->Y
            cin >> u >> v;
            if (flip) swap(u,v);
            if (comp) {
                adjMtx[u].set(v);
                compAdjMtx[u].reset(v);

                transAdjMtx[v].set(u);
                compTransAdjMtx[v].reset(u);
            } else {
                adjMtx[u].reset(v);
                compAdjMtx[u].set(v);

                transAdjMtx[v].reset(u);
                compTransAdjMtx[v].set(u);
            }
        } elif (cmd==5) { // transpose (flip edge direction)
            flip=!flip;
        } else { // complement
            comp=!comp;
        }
    }

    // number of vertices
    cout << V << '\n';
    auto& ans = [&]() -> vector<bitset<4005>>& {
        if (flip) return comp ? compTransAdjMtx : transAdjMtx;
        return comp ? compAdjMtx : adjMtx;
    }();

    // outdegree of vi, and the hash of the adjl of vi %10^9 + 7
    int shift=4005-V;
    FOR(i,0,V) {
        ans[i]<<=shift;
        ans[i]>>=shift;
        ans[i].reset(i);
        cout << ans[i].count() << ' ';
        ll hans=0;
        ll pw=1;
        for (size_t j=ans[i]._Find_first(); j < (size_t)V; j = ans[i]._Find_next(j)) {
            hans = (hans + (pw * j)) % MOD;
            pw = (pw * 7) % MOD;
        }
        cout << hans << '\n';
    }
}