...
signed main() {
    ...
    ll s,p,m,t, ans=LLINF; int n; cin >> s >> p >> m >> n;
    int lp=0;
    vll trips(n);
    vll ptr(n); // ptr to end range
    FOR(i,0,n) {
        cin >> trips[i]; // sorted
        while (trips[i]>(trips[lp]+m-1)) {
            ptr[lp]=i-1; lp++;
        }
    } 
    FOR(i,lp,n) {
    ...
    trips.assign(n,LLINF); trips[0]=s; trips[ptr[0]]=min(trips[ptr[0]],p);
    FOR(i,1,n) {
        trips[i]=min(trips[i-1]+s,trips[i]);
        if (trips[i-1]!=LLINF) trips[ptr[i]]=min(trips[ptr[i]], trips[i-1]+p);
    }
    cout << trips[n-1] << '\n';
}