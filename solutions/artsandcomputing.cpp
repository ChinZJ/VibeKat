...
signed main() {
    ...
    int m = static_cast<int>(stu.size());
    constexpr int INF = 1e9;

    // memo[i] = min pairs to seat students 0..i-1, -1 = unvisited
    vector<int> memo(m + 1, -1);

    // Top-down: solve(i) = min seat-pairs to place first i students
    auto solve = [&](auto& self, int i) -> int {
        ...
        // Option 1: cur sits alone (only A can sit without a C buddy)
        if (cur == 'A') {
            res = min(res, self(self, i - 1) + 1);
        }

        // Option 2: pair cur with previous student
        if (i >= 2) {
            char prev = stu[i - 2];
            if (!(cur == 'C' && prev == 'C')) {
                res = min(res, self(self, i - 2) + 1);
            }
        }
    ...
    // Backtrack — same logic, just query the memoized values
    cout << "YES\n";
    ...
    while (i > 0) {
        ...
        if (i >= 2) {
            char prev = stu[i - 2];
            if (!(cur == 'C' && prev == 'C') &&
                solve(solve, i) == solve(solve, i - 2) + 1) {
                pairs.push_back({prev, cur});
                i -= 2;
                continue;
            }
        }
    ...
    for (int j = static_cast<int>(pairs.size()); j < n; ++j) {
        result += "..";
    }
