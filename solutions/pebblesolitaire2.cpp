...
signed main() {
    ...
    auto solve = [&](auto& self, int board) -> int {
        if (memo.find(board) != memo.end()) return memo[board];

        int best=__builtin_popcount(board);
        // Check right ward
        for (int i=0; i+2<23; ++i) {
            if (!(board >> i & 1) 
                    && (board >> (i+1) & 1 && (board >> (i+2) & 1))) {
                int next = board ^ (1 << i) ^ (1 << (i+1)) ^ (1 << (i+2));
                best = min(best, self(self,next));
            }
            if (!(board >> (i+2) & 1)
