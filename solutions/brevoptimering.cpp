...
signed main() {
    ...
    int N,k,j,w, u,v; cin >> N;
    double M, I, U;
    vector<double> Ms(N);
    vector<vector<pii>> AL(N); // neigh,weight
    vi cnts(N);
    FOR(i,0,N) {
        cin >> Ms[i];
        cin >> k;
    ...
    while (!q.empty()) {
        ...
        M = Ms[u]; I=inputs[u];
        U=min(M,I);
        if (I-M>EPS && M-I<=EPS) ans.pb(u);

        for(auto& [neigh, weight] : AL[u]) {
            inputs[neigh]+=(U*weight/100);
            if (--cnts[neigh]==0) q.push(neigh);
        }
    }
    sort(ALL(ans));
    FOR(aa,0,sz(ans)) cout << ans[aa]+1 << ' ';
    cout << '\n';
}