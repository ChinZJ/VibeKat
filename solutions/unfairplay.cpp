...
signed main() {
    ...
    while (cin >> N && N!=-1) {
        ...
        FOR(i,0,M) {
            cin >> a >> b;
            if (a==N) {res[i]=-2; score[N-1]+=2;}
            elif (b==N) {res[i]=-3; score[N-1]+=2;}
            else {
                tot++;
                flow.addEdge(src,2+i,2);
                flow.addEdge(2+i,2+M+a-1,2);
                flow.addEdge(2+i,2+M+b-1,2);
                res[i]=a;
            }
        }
        ...
        FOR(i,0,N-1) {
            ...
            flow.addEdge(2+M+i, snk, nmax-1-(score[i]));
        }   
        
        if (!ok) cout << "NO\n";
        elif (flow.calc(src,snk) != (tot*2)) {
            cout << "NO\n";
        } else {
            FOR(i,0,M) {
                if (res[i]==-2) {
