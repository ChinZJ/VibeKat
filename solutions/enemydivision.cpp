...
signed main() {
     ...
     * Split by groups.
     * 
     * Observe that if degree <= 1 only 1 group suffices
     * 
     * Else a greedy allocation with flipping will suffice
     */
    ...
    FOR(i,0,m) {
        ...
        maxdeg = max(maxdeg, sz(adj[a]));
        maxdeg = max(maxdeg, sz(adj[b]));
    }
    ...
    while (!unsat.empty()) {
        int u = unsat.front(); unsat.pop();
        int tot=0;
        for (int v : adj[u]) {
            if (tot > 1) break;
            if (assignment[u] == assignment[v]) ++tot;
        }
        if (tot > 1) {
            assignment[u] = (assignment[u]+1)%2;
            for (int v : adj[u]) {
                unsat.push(v);
            }
        }
    ...
    cout << 2 << '\n';
    REP(i,1,n) {
        if (assignment[i] == 0) cout << i << ' ';
    }
    cout << '\n';
    REP(i,1,n) {
        if (assignment[i] == 1) cout << i << ' ';
    }
    cout << '\n';
}