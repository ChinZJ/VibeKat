...
signed main() {
    ...
    ld t1,t2,t3,t4, tend; cin >> t1 >> t2 >> t3 >> t4 >> tend;
    tend*=3;
    ld tot=t1+t2+t3+t4;

    ld worst=max({t1,t2,t3,t4});
    ld best=min({t1,t2,t3,t4});

    if (tot-best <= tend+EPS) {cout << "infinite\n"; return 0;}
    if (tot-worst > tend+EPS) {cout << "impossible\n"; return 0;}
    ...
    while (hi-lo >= EPS) {
        ...
        ws=max(worst,mid);
        bs=min(best,mid);
        if (tot+mid-ws-bs <= tend+EPS) lo=mid;
        else hi=mid;
    }
    // if (lo < 1.00-EPS) {cout << "impossible\n"; return 0;}
    // if (lo > 20.00+EPS) {cout << "infinite\n"; return 0;}
