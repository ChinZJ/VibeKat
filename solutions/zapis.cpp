...
signed main() {
    ...
    auto ways = [&](int i, int k) -> ll {
        if (s[i] == '?' && s[k] == '?') return 3;  // (), [], {}
        if (s[i] == '?' && s[k] == ')') return 1;   // must be ()
        if (s[i] == '?' && s[k] == ']') return 1;   // must be []
        if (s[i] == '?' && s[k] == '}') return 1;   // must be {}
        if (s[i] == '(' && s[k] == '?') return 1;   // must be ()
        if (s[i] == '[' && s[k] == '?') return 1;   // must be []
    ...
    auto dp=[&](auto& self, int i, int j) -> ll {
        ...
        for (int k=i+1; k<=j; k+=2) {
            ll tmp=ways(i,k);
            ll left=self(self, i+1, k-1);
            if (left>=MOD) {
                tog=true;
                left%=MOD;
            }
            ll right=self(self, k+1, j);
            if (right>=MOD) {
                tog=true;
                right%=MOD;
            }
    ...
    ll res=dp(dp,0,N-1);
    if (tog) cout << setfill('0') << setw(5) << res <<'\n';
    else cout << res << '\n';
}