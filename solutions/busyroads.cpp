...
signed main() {
    ...
    FOR(i,0,M) {
        int A,B; ll T,L,R; cin >> A >> B >> T >> L >> R;
        adj[A].pb({B,T,L,R});
        adj[B].pb({A,T,L,R});
    }
    ...
    pq.push({0LL,1});
    vll vis(N+1,LLONG_MAX);
    while (!pq.empty()) {
        auto [tott, city] = pq.top(); pq.pop();
        if (tott >= vis[city]) continue;
        if (city == N) {cout << tott << '\n'; return 0;}

        // cout << "visiting " << tott << " at city " << city << '\n';
        ...
        for (auto& e : adj[city]) {
            ...
            if (tott > e.r) {
                ll a = (tott - e.r + C - 1) / C;
                ntott = max(tott, (e.l + (C*a))) + e.t;
            } else ntott = max(tott, e.l) + e.t;
            // cout << "  status " << ntott << ' ' << " from " << city << " to " << e.b << " with range " << e.l << ' ' << e.r << '\n';
            if (ntott < vis[e.b]) {
                // cout << "  push\n";
                pq.push({ntott, e.b});
            }
