...
signed main() {
    ...
    vector<pii> mv{{0,1},{1,0},{-1,0},{0,-1}};

    vector<vi> vis(N,vi(M));

    priority_queue<Tile, vector<Tile>, greater<Tile>> pq; pq.push({sx,sy,0,K});

    Tile t; int nx, ny, cst;
    while (!pq.empty()) {
        t=pq.top(); pq.pop();
        ...
        for (auto [xx,yy] : mv) {
            ...
            if (nx>=0 && nx<N && ny>=0 && ny<M && grid[nx][ny]!='#') {
                ...
                if (t.lk>=cst) {
                    pq.push({nx,ny,t.tr,t.lk-cst});
                } elif (K>=cst) {
                    pq.push({nx,ny,t.tr+1,K-cst});
                }
