...
signed main() {
    ...
    while (eol) {
        string s;
        int uid=0;
        vi projs; projs.reserve(101); projs.pb(0);
        unordered_map<string,int> pnames;
        unordered_map<int,string> revpnames;

        int sid=-1;
        unordered_map<string,int> snames;
        unordered_map<int,string> revsnames;
        vi studs(10000,0);
        string currproj;
        while(getline(cin,s)) {
            if (s=="0") {
            ...
            if (s=="1") {
                vector<pair<int,string>> outs(sz(projs)-1);
                FOR(i,1,sz(projs)) {
                    outs[i-1]={-projs[i],revpnames[i]};
                }
                sort(ALL(outs));
                for (auto& [a,b] : outs) cout << b << ' ' << -a << '\n';
                break;
            } 
            
            elif (isupper(s[0])) { // proj
                ++uid;
                pnames[s]=uid;
                revpnames[uid]=s;
                projs.pb(0);
                ...
                if (studs[sidx]==-1 || studs[sidx]==uid) {
                    continue;
                }
                if (studs[sidx]!=0) {
                    projs[studs[sidx]]--;
