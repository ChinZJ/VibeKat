...
signed main() {
    ...
    vll arr(n);
    FOR(i,0,n) cin >> arr[i];
    ll maxspace=arr[n-1]+arr[n-2];
        
    vll greed(maxspace+1); ll cnt=0; ll tmp=0, toadd=0;
    REP(val,1,maxspace) {
        cnt=0; tmp=val;
        REF(i,n-1,0) {
    ...
    vll dp(maxspace+1,LLINF); dp[0]=0;
    ll rem=0;
    bool canon=true;
    REP(val,1,maxspace) {
        FOR(i,0,n) {
            if (arr[i] <= val && dp[val-arr[i]]!=INF) {
                dp[val]=min(dp[val], dp[val-arr[i]]+1);
            }
        }
        if (dp[val]<greed[val]) {
            canon=false; break;
