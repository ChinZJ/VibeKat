...
struct Dinic {
    ...
    T calc(int s, int t, bool first = true) {
        ...
        if (first) {
            ...
            if (max_cap > 0)
                start_bit = 63 - __builtin_clzll(max_cap);
        }

        for (int bit = start_bit; bit >= 0; bit--) { // Scaling optimization
            ...
            do {
                ...
                while (qi < qe && !lvl[t]) {
                    ...
                    for (Edge& e : adj[v]) { // Note that for each iteration, edge is constantly being updated, previous paths will not be considered
                        if (!lvl[e.to] && (e.c >> bit)) { // simple check will just be e.c > 0 instead of optimization
                            q[qe++] = e.to;
                            lvl[e.to] = lvl[v] + 1;
                        }
                    }
    ...
    // Min cut boundary is where reachable meets unreachable
    // Returns true if vertex i is in the S set, else flase if in the T set
...
/**
 * Lower bound: force the network to push at least that much flow through the edge
 * Done using a super source and super sink
 * 
 * Imagine instantly forcing the minimum required flow L(u,v) (lower bound) through every edge
 *      The edge u -> v now has remaining capacity of U(u,v) - L(u,v) (upper bound - lower bound)
 *          Node u now has a flow deficit
 *          Node v now has a flow surplus
 *          To make such a network valid, 
 *              Supply the deficit nodes with extra flow
 *              Drain surplus from excess nodes
 * 
 * For every directed edge u -> v with lower bound L and upper bound U
 *      1. addEdge(u, v, U - L)
 *      2. Update demands demand[u] -= L (lost flow)
 *                        demand[v] += L (gain flow)
 *      3. Add two nodes S' and T' to serve as super source and super sink
 *         if demand[i] > 0     addEdge(S', i, demand[i]) 
 *         if demand[i] < 0     addEdge(i, T', -demand[i])
 *      4. Connect original sink to original source 
 *         addEdge(T, S, INF)
 *              The amount of water flowing through T -> S is the amount of valid flow 
 *              thtat traverse real network from S -> T to satisfy minimum bounds
 * 
 *      5. Check feasibility by runnning S' to T'
 *         Calculate sum of all positive demands = total flow for it to be satisfied
 * 
 *      6. If satisfied, remove the artificial infinite edge from T to S (set residual capacity to 0 via flow.updateEdge(xxx, 0))
 *         Run Dinic from S to T this time
 *         Final max flow is the flow pushed through the T -> S infinite edge + additional flow found in this step (running flow.calc(S,T))
 * 
 *      7. To get minimum flow instead run calc(T, S) to return flow
 *         True minimum flow is the base flow - removable flow
 */

const ll MOD1 = 998244353;
const ll MOD2 = 1000000007;

ll mergeCount(vector<int>& arr, int l, int mid, int r) {
...
signed main() {
     ...
     * TODO
     * 
     * Observe that for 500 words, we can easily check swappable by counting inversions
     * 
     * Use a sorted string as parity to construct a bipartite graph.
     * Then by Konigs theorem, we have max independnt set = n - min vertex cover
     */


    int n; cin >> n;

    int src=n+1, snk=src+1;
    Dinic<ll> flow(snk+1);

    vector<string> ss(n); string orig; 
    vi invs(n,0);
    ...
    FOR(i,0,n) {
        ...
        vi tmpp(sln);
        FOR(j,0,sln) {
            tmpp[j] = mapper[tmp[j] - 'a'];
        }
        int inv=countInversions(tmpp, 0, sln-1);
        if (inv%2 == 0) flow.addEdge(src,i,1);
        else flow.addEdge(i,snk,1);
        invs[i]=inv%2;
    }
    ...
    FOR(i,0,n) {
        ...
        FOR(j,i+1,n) {
            ...
            if (diff==2) {
                if (invs[i]==0) flow.addEdge(i,j,1);
                else flow.addEdge(j,i,1);
            }
        }
    }
