...
signed main() {
     ...
     * 
     * 
     * Consider node 1 first
     * For any i,j (that is not 1) 
     *      If A[i][j] = 1, then node 1 sits on the path between i and j
     *          i,j belong in different subtrees
     *      If A[i][j] > 1, then they are in the same subtree
     * 
     * Given a set of nodes, find the minimum
     * Partition the remaining nodes into groups
     * Connect the minimum to one node per group
    ...
    auto solve = [&] (auto& self, vi& nodes) -> void {
        if (sz(nodes) <= 1) return;
        
        int m = *(min_element(nodes.begin(), nodes.end()));
        
        // Partition remaining nodes into groups
        // Two nodes u,v are in the same group iff A[u][v] > m
        vector<vi> groups;
        for (int u : nodes) {
            if (u==m) continue;
        ...
        // Connect m to the min of each group and recurse
        for (auto& g : groups) {
    ...
    vi st(N); iota(st.begin(), st.end(), 1);
    solve(solve, st);
