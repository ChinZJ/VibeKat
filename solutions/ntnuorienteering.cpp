...
signed main() {
    ...
    int vis[1001];
    int dp[1001];
    int C,L,i,j; ll li,lj, t;
    while (T--) {
        cin >> C >> L;

        vector<vll> dist(C,vll(C, LLINF));
        FOR(a,0,C) dist[a][a]=0;
        FOR(a,0,(C*(C-1)/2)) {
            cin >> i >> j >> t;
        ...
        vector<tuple<ll,ll,int>> lecs(L); // end,start,loc
        FOR(a,0,L) {
            cin >> i >> li >> lj; // id,start,end
            lecs[a]={lj,li,i};
        }
        sort(ALL(lecs));
        ...
        int hi=0;
        FOR(a,0,L) {
            auto [lend,lstart, lloc]=lecs[a];
            vis[a]=T; dp[a]=1;
            REF(b,a-1,0) {
                auto [pend,pstart,ploc]=lecs[b];
                ll incur=dist[lloc][ploc];
                if (pend+incur<=lstart && vis[b]==T) {
