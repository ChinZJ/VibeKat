...
signed main() {
    ...
    while (T--) {
        ...
        vector<multiset<pair<int,ll>>> stn(N+1);
        ...
        queue<int> qdeg;
        REP(i,1,N) {
            if (sz(stn[i])==2) qdeg.push(i);
        }

        while (!qdeg.empty()) {
            int rx=qdeg.front();qdeg.pop();
            if (deg[rx]!=2) continue;

            auto it=stn[rx].begin();
            auto [d,e]=*it;
            ++it;
            auto [f,g]=*it;

            if (rx==d && d==f) continue;

            // cout << "processing " << rx << '\n';
            // cout << d << " d " << e << " e " << f << " f " << g << " g\n";

            stn[d].erase(stn[d].find({rx,e}));
            stn[f].erase(stn[f].find({rx,g}));
            ...
            // cout << d << " has deg " << deg[g] << " and " << f << " has " << deg[f] << '\n';

            if (deg[d]==2) qdeg.push(d);
            if (deg[f]==2) qdeg.push(f);
