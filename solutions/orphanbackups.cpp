...
signed main() {
    ...
    while (getline(cin,inpt)) {
        int lim=inpt.rfind('_');
        int lim2=inpt.rfind('_',lim-1);
        stp=inpt.substr(0,lim2);
        if (nm.find(stp)==nm.end()) orf.pb(inpt);
        else hm[stp]++;
    }
    bool sol=false;
    sort(ALL(orf));
