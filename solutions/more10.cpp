...
signed main() {
    ...
    vector<pii> isset; isset.reserve(N*2);
    vector<pii> notset; notset.reserve(N*2);
    vector<string> uqwords; uqwords.reserve(N*2);
    int uid=0; unordered_map<string,int> name; unordered_map<int,string> revname;

    string s1,s2,s3;

    unordered_map<string,vi> suff1,suff2,suff3;
    vector<string> oneC,twoC; oneC.reserve(N*2); twoC.reserve(N*2);

    FOR(i,0,N) {
        cin >> s1 >> s2 >> s3;
        auto addName = [&](string& nme) {
            if (name.find(nme)==name.end()) {
                uqwords.pb(nme);
                name[nme]=uid;
                revname[uid]=nme;
                int len=sz(nme);
                if (sz(nme)==1) {
                    ...
                    twoC.pb(nme);
                    suff1[nme.substr(1)].pb(uid);
                    suff2[nme].pb(uid);
                } else {
                    suff1[nme.substr(len-1)].pb(uid);
                    suff2[nme.substr(len-2)].pb(uid);
                    suff3[nme.substr(len-3)].pb(uid);
                }
                uid++;
            } 
        };
        addName(s1); addName(s3);
        
        if (s2=="is") isset.pb({name[s1], name[s3]});
        else notset.pb({name[s1], name[s3]});
    }

    DSUExtended dsu(sz(uqwords));
    for(auto [a,b] : isset) {
        dsu.unite(a,b);
    }
    ...
    for (auto [a,b] : notset) {
        if (dsu.same(a,b)) {cout << "wait what?\n"; return 0;}
    }
    cout << "yes\n";
