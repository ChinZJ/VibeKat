...
signed main() {
    ...
    /**
     * By construction, this is a functional graph.
     *      Every node has exactly one outgoing edge.
     *      Thus each node can belong to at most one cycle
     * 
     * Perform coloring DFS with 3 states
     *      0 - unvisited
     *      1 - currently exploring
     *      2 - fully processed
     * 
     * The answer will be sum of 
     *      - nodes with in degree 0 (receive no gifts)
     *      - pure cycle components (every node is on the cycle)
     */
    ...
    for (int i=0; i<N; ++i) {
        ...
        if (color[cur] == 1) {
            ...
            do {
                ...
                cycNode = gift[cycNode];
            } while (cycNode != cur);

            if (pure) ++purecycle;
        }
        for (int node : path) color[node] = 2;
        
    }

    cout << (nogift==0 && purecycle == 1 
        ? 0
        : nogift + purecycle) << '\n'; 
