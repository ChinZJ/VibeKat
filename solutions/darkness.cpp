...
signed main() {
    ...
    cin.ignore();
    string s;
    double ss; double z2=H*H;
    FOR(i,0,R) {
        getline(cin,s);
        FOR(j,0,C) {
            ss=(double)(s[j]-'0');
            FOR(k,0,R) {
                double x2=(abs(i-k)); x2*=x2;
                FOR(l,0,C) {
                    double y2=(abs(j-l)); y2*=y2;
                    grid[k][l]+=(ss/(x2+y2+z2));
                }
            }
        }
    }

    vector<vector<bool>> gridb(R,vector<bool>(C));
    FOR(i,0,R) {
        FOR(j,0,C) {
            if (grid[i][j]<B) gridb[i][j]=0;
            else gridb[i][j]=1;
        }
    }
    ...
    vector<pii> mv={{0,1},{1,0}};
    int nx,ny,ncst;
    FOR(i,0,R) {
        FOR(j,0,C) {
            // Border
            if (i==0 || i==R-1 || j==0 || j==C-1) {
                flow.addEdge(src,(C*i)+j,LLINF, LLINF);
            }
            // Dark
            if (gridb[i][j]==0) {
                flow.addEdge((C*i)+j,snk,LLINF,LLINF);
            }
            for (auto [xx,yy] : mv) {
                nx=xx+i; ny=yy+j;
                if (nx>=0 && nx<R && ny>=0 && ny<C) {
                    ncst=gridb[i][j]+gridb[nx][ny];
                    if (ncst==2) { // bright bright
                        flow.addEdge((C*i)+j, (C*nx)+ny, 43, 43);
                    } elif (ncst==1) { // bright dark
                        flow.addEdge((C*i)+j, (C*nx)+ny, 11, 11);
                    }
                    // dark dark fallthrough
                }
            }
        }
    }
